...
export class UserService {
  ...
  async sendResetPasswordEmail(user) {
    ...
    const template = compile(fs.readFileSync('templates/reset-password.template', 'utf8'));
    const html = template({
  ...
  async update(toUpdate: UserEntity, dto: UserUpdateDto): Promise<IUser> {
    toUpdate.createdBy = this.request.user._id;
    toUpdate.updatedBy = this.request.user._id;
