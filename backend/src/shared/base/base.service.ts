...
> implements IBaseService<T, createDto, updateDto>
{
  ...
  async findAll(condition = { isDeleted: false } as FindConditions<T>): Promise<T[]> {
    const where: FindConditions<T> = { ...condition };
    return this.repository.find({ where });
  }

  async paginate(take, skip, condition = { isDeleted: false }, type?): Promise<any> {
    const queryTake = Number(take) || PaginationConstants.TAKE;
    const querySkip = Number(skip) || PaginationConstants.SKIP;
  ...
  async create(data: createDto): Promise<T> {
    ...
    if (this.request.user) {
      newEntity.createdBy = this.request.user._id;
      newEntity.updatedBy = this.request.user._id;
    }
  ...
  async updateStatus(_id: ObjectID, isDeleted: boolean): Promise<T> {
    ...
    if (this.request.user) {
      entity.updatedBy = this.request.user._id;
    }
  ...
  async search(data: QueryDto<T>): Promise<SearchResponse<T>> {
    ...
    const queryTake = +data.take || PaginationConstants.TAKE;
    const querySkip = +data.skip || PaginationConstants.SKIP;
